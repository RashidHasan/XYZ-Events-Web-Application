...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['name'], $_POST['password'], $_POST['repeat-password'], $_POST['email'], $_POST['phone'])) {
        ...
        if (empty($name) || empty($password) || empty($repeatPassword) || empty($email) || empty($phoneNumber)) {
            ...
            $checkStmt = $connection->prepare("SELECT COUNT(*) FROM attendee WHERE email = ? AND password = ?");
            ...
            if ($row > 0) {
                ...
                $stmt = $connection->prepare("INSERT INTO attendee (name, password, email, phoneNumber) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $name, $password, $email, $phoneNumber);
                if ($stmt->execute()) {
                    echo "<script>alert('Sign-up successful!'); window.location.href='attendee-register.php';</script>";
                } else {
        ...
        $checkStmt = $connection->prepare("SELECT ID, COUNT(*) FROM attendee WHERE email = ? AND password = ?");
        $checkStmt->bind_param("ss", $email, $password);
        $checkStmt->execute();
        $checkStmt->bind_result($attendeeID, $row);
        $checkStmt->fetch();
        $checkStmt->close();

        if ($row > 0) {
            $_SESSION['attendeeID'] = $attendeeID;
            echo "<script>alert('Login successful!'); window.location.href='attendee-home.php';</script>";
        } else {
    ...
    <main>
        <nav class="navbar navbar-expand-lg">
            <div class="container">
                ...
                <a href="index.php" class="btn custom-btn d-lg-none ms-auto me-4">Go Back</a>
            ...
            <div class="cardDiv">
                <div class="exotic-container" id="containerCard">
                    <div class="fantasy-form-container sign-up">
                        <form method="POST" action="attendee-register.php">
